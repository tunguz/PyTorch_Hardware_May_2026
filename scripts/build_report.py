...
def pdf_inline(text: str) -> str:
    ...
    text = re.sub(
        r"https?://[^\s<]+",
        lambda match: f'<link href="{match.group(0)}"><font color="#4f46e5">{match.group(0)}</font></link>',
        text,
    )
...
- `requirements.txt` - Python dependencies for rebuilding generated artifacts
- `README.md` - project overview, license, and citation information

## Rebuilding Generated Artifacts

The static report artifacts are generated from `deep-research-report.md`, the
source infographic, and `scripts/build_report.py`. From a fresh checkout:
...
The build script updates `index.html`, `README.md`, `infographic.jpeg`,
`backend-position-map.png`, and `State of PyTorch Hardware Acceleration May
2026.pdf`. The Word document is a checked-in reference artifact and is not
